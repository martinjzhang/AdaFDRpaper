...
def main(args):    
    ...
    ## report the baseline   
    n_rej,t_rej=pf.bh(p,alpha=alpha,n_sample=n_full,verbose=False)
    logger.info('## BH, n_rej=%d, t_rej=%0.5f'%(n_rej,t_rej))
    n_rej,t_rej,pi0_hat=pf.storey_bh(p,alpha=alpha,n_sample=n_full,verbose=False)
    logger.info('## SBH, n_rej=%d, t_rej=%0.5f'%(n_rej,t_rej))
    
    ## feature explore function
    pf.feature_explore(p,x,alpha=alpha,vis_dim=None,cate_name=cate_name,\
    ...
    if use_cv:
        n_rej,t,_=pf.PrimFDR_cv(p,x,5,alpha=alpha,h=h,n_itr=n_itr,verbose=True,\
        ...
        n_rej,t,_=pf.PrimFDR(p,x,5,alpha=alpha,h=h,n_itr=n_itr,verbose=True,\
                             output_folder=output_folder,logger=logger)
